...
function startSimulation() {
  ...
  const leftCliff = Bodies.rectangle(CHASM_LEFT / 2, GROUND_Y + 70, CHASM_LEFT, 140, {
...
function simulationStep() {
  ...
  for (const [beamId, bp] of beamPhys) {
    ...
    if (stretch > bp.material.breakDistance) {
...
function spawnWave(idx) {
  ...
  for (let i = 0; i < wave.count; i++) {
    ...
    World.add(engine.world, body);
    vehicles.push({ body, wave: idx, state: 'active', color: wave.color, speed: wave.speed });
  }
}
...
function drawVehicles() {
  for (const v of vehicles) {
    ...
    ctx.rotate(b.angle);
    const verts = b.vertices.map(vt => ({ x: vt.x - b.position.x, y: vt.y - b.position.y }));
